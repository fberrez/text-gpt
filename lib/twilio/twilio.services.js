...
class TwilioServices {
	static async processWhatsAppMessage(from, message) {
		...
		if (!await UserServices.userIsValid(user)) {
			return this.sendWhatsAppMessage(from, 'You have to subscribe to use this chatbot. Send "subscribe" to subscribe.');
		}
	...
	static async startCreateUserScenario(phoneNumber, message) {
		...
		const user = await UserServices.createUser(formatWhatsappNumber(phoneNumber));
